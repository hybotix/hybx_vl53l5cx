...
uint32_t hybx_signal_per_spad[8][8];
uint16_t hybx_range_sigma_mm[8][8];
...
void hybx_vl53l5cx::_readFrame()
{
    ...
    for (uint8_t i = 0; i < zones; i++) {
        uint8_t row = i / 8;
        uint8_t col = i % 8;
        hybx_distance_mm[row][col]    = results.distance_mm[i];
        hybx_target_status[row][col]  = results.target_status[i];
        hybx_signal_per_spad[row][col] = results.signal_per_spad[i];
        hybx_range_sigma_mm[row][col]  = results.range_sigma_mm[i];
    }
